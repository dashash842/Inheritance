...
public:
	...
	virtual std::ostream& info(std::ostream& os)const
	{
		...
		//strchr() в указаной строке (str) находит указаный символ(symbol),
		//и возвращает указатель на найденный символб или 'nullptr',
		//если указанный символ отсутсвует в указаной строке.
		//class Student;
		//return os << last_name << " " << first_name << " " << age;
		os.width(NAME_WIDTH); //метод width задает размер поля, в которое будет выделено значение;
		os << last_name;
		os.width(NAME_WIDTH);
		os << first_name;
		os.width(AGE_WIDTH);
...
public:
	...
	std::istream& scan(std::istream& is)override
	{
		...
		is.read(sz_buffer, SPECIALITY_WIDTH);// fin.read читает заданное количество символов из файла и сохраняет их в NTL (NULL-terminate lINE)
		 
		//удаляем лишние пробелы в конце прочитанной строке
		for (int i = SPECIALITY_WIDTH; sz_buffer[i] == ' '; i--)
			sz_buffer[i] = 0;
	    
		//Удаляем лишние пробелы в начале прочитаной строки:
		while (sz_buffer[0] == ' ')
...
void main()
{
	...
	Graduate graduate("Tven", "Mark", 22, "Chemistry", "WW_220", 96, 99, "Pogramm");
	graduate.info();
#endif // INHERITANCE
	...
	Human* group[] =
	{
		...
		new Graduate("Tven", "Mark", 22, "Chemistry", "WW_220", 96, 99, "Pogramm"),
